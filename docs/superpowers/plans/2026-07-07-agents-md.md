...
**Goal:** Create `/AGENTS.md` at the repo root containing the nine-section development-workflow guide defined in the spec, so any fresh-session agent can self-onboard and follow consistent conventions.

**Architecture:** A single static Markdown file. No code, no tests. The file is authored in nine sections per the spec's "Order in the final file," then committed. Verification is structural (sections present, paths/commands match the real repo) and editorial (reads coherently top-to-bottom).

**Tech Stack:** Markdown / CommonMark (GitHub-flavored, rendered by opencode CLI).

**Spec:** `docs/superpowers/specs/2026-07-07-agents-md-design.md`

---

## File Structure

- Create: `AGENTS.md` (repo root — the conventional location opencode and most agent tools read automatically)

No other files are created or modified. No tests — this is documentation. The existing test suite (`pytest`) is unaffected.
...
Run:
```bash
git checkout main && git pull && git checkout -b feature/agents-md
```

Expected: `Switched to a new branch 'feature/agents-md'`. If `git pull` fails because there's no upstream tracking, that's fine — proceed.

- [ ] **Step 2: Confirm branch**

Run: `git branch --show-current`
Expected: `feature/agents-md`

---

### Task 2: Write the AGENTS.md file (title + Section 1: Standard Workflow)

**Files:**
- Create: `AGENTS.md`

- [ ] **Step 1: Create AGENTS.md with the title and the Standard Workflow checklist**

Write the following exact content to `AGENTS.md`:
...
- [ ] **Step 2: Verify the file exists and the title is present**

Run: `head -5 AGENTS.md`
Expected: starts with `# AGENTS.md`, then the intro paragraph, then `## Standard Workflow (read this first, every session)`.

---

### Task 3: Append Section 2 — Project Overview

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Project Overview section**
...
Run: `grep -c "## Project Overview" AGENTS.md`
Expected: `1`

Run: `grep -c "### Key constraint" AGENTS.md`
Expected: `1`

---

### Task 4: Append Section 3 — Setup & Commands

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Setup & Commands section**
...
Run: `grep -c "## Setup & Commands" AGENTS.md`
Expected: `1`

Run: `grep -c "browser-use --doctor" AGENTS.md`
Expected: `1`

---

### Task 5: Append Section 4 — Architecture Map

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Architecture Map section**
...
Run: `grep -c "## Architecture map" AGENTS.md`
Expected: `1`

Run: `grep -c "Three things that trip people up" AGENTS.md`
Expected: `1`

---

### Task 6: Append Section 5 — Browser-Use Manual Playbook

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Browser-Use Manual Playbook section**
...
Run: `grep -c "## Browser-Use Manual Playbook" AGENTS.md`
Expected: `1`

Run: `grep -c "When to stop and ask the user" AGENTS.md`
Expected: `1`

---

### Task 7: Append Section 6 — Browser-Use Scripted Patterns

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Browser-Use Scripted Patterns section**
...
Run: `grep -c "## Browser-Use Scripted Patterns" AGENTS.md`
Expected: `1`

Run: `grep -c "Canonical shape (lifted from scraper.py)" AGENTS.md`
Expected: `1`

---

### Task 8: Append Section 7 — Branch Workflow

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Branch Workflow section**
...
Run: `grep -c "## Branch Workflow" AGENTS.md`
Expected: `1`

Run: `grep -c "When NOT to merge" AGENTS.md`
Expected: `1`

---

### Task 9: Append Section 8 — Feature Process

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Feature Process section**
...
Run: `grep -c "## Feature Process" AGENTS.md`
Expected: `1`

Run: `grep -c "Where to read state mid-flow" AGENTS.md`
Expected: `1`

---

### Task 10: Append Section 9 — Picking Up In-Flight Work

**Files:**
- Modify: `AGENTS.md` (append at end)

- [ ] **Step 1: Append the Picking Up In-Flight Work section**
...
Run: `grep -c "## Picking Up In-Flight Work" AGENTS.md`
Expected: `1`

Run: `grep -c "Resume the correct loop" AGENTS.md`
Expected: `1`

---

### Task 11: Structural verification — all nine sections present

**Files:**
- (no changes — verification only)

- [ ] **Step 1: Count all top-level workflow sections**

Run:
```bash
grep -E "^## (Standard Workflow|Project Overview|Setup & Commands|Architecture map|Browser-Use Manual Playbook|Browser-Use Scripted Patterns|Branch Workflow|Feature Process|Picking Up In-Flight Work)" AGENTS.md | wc -l
```
Expected: `9`

If the count is not 9, a section header is missing or mistitled — fix before
proceeding.
...
Run:
```bash
grep -E "TBD|TODO|FIXME|fill in|implement later" AGENTS.md
```
Expected: no output (empty). If any line matches, fix the placeholder.

- [ ] **Step 3: Verify referenced repo paths actually exist**

These paths are mentioned in AGENTS.md and must match the real repo:
```bash
test -f scraper.py && echo "scraper.py OK"
test -f app.py && echo "app.py OK"
test -f progress_tracker.py && echo "progress_tracker.py OK"
test -f backend/server.py && echo "backend/server.py OK"
test -f backend/scraper_session.py && echo "backend/scraper_session.py OK"
test -f frontend/app.js && echo "frontend/app.js OK"
test -f run.sh && echo "run.sh OK"
test -f requirements.txt && echo "requirements.txt OK"
test -d tests && echo "tests/ OK"
test -d docs/superpowers/specs && echo "docs/superpowers/specs/ OK"
test -d docs/superpowers/plans && echo "docs/superpowers/plans/ OK"
test -f docs/superpowers/specs/2026-07-05-ui-overhaul-design.md && echo "ui-overhaul spec OK"
```
Expected: every line prints `OK`. If any path fails, the AGENTS.md reference
is wrong — fix the path in AGENTS.md to match the real repo, or note the
discrepancy for the user.

- [ ] **Step 4: Verify the merge commands reference the real branch name**

The Branch Workflow section shows `feature/<slug>` and `fix/<slug>` as
placeholders for the *pattern* — that's correct and intentional (they're
templates, not literal branch names). Confirm no literal `<slug>` appears
anywhere it would be mistaken for a real branch:

Run:
```bash
grep -n "<slug>" AGENTS.md
```
Expected: every match is inside the Branch Workflow section's template
examples (lines containing `feature/<slug>` or `fix/<slug>`). No `<slug>`
should appear in commands presented as runnable verbatim outside those
templates.

- [ ] **Step 5: Verify the file reads coherently end-to-end**

Run: `wc -l AGENTS.md`
Expected: roughly 280–320 lines (single file, nine sections).

Read the file top-to-bottom once. Confirm:
- Title and intro paragraph precede Section 1.
- The nine sections appear in the spec's order.
- No section is empty or truncated.
- Cross-references between sections (e.g. "See Branch Workflow", "See
  Browser-Use Manual Playbook") point at sections that actually exist.

---

### Task 12: Commit the AGENTS.md file
...
```bash
git add AGENTS.md
git commit -m "docs: add AGENTS.md development-workflow guide"
```
Expected: a single commit is created on `feature/agents-md`.

- [ ] **Step 2: Confirm the commit**

Run: `git log --oneline -3`
Expected: the newest commit is `docs: add AGENTS.md development-workflow
guide` on branch `feature/agents-md`.

Run: `git show --stat HEAD`
Expected: one file changed (`AGENTS.md`), ~300 insertions.

- [ ] **Step 3: Confirm main is untouched**

Run: `git log --oneline main -1`
Expected: main is still at `Add AGENTS.md design spec` (or whatever the
latest main commit was before branching). The new AGENTS.md commit is only
on `feature/agents-md`.

---

### Task 13: Hand off for manual verification

**Files:**
- (no changes — handoff only)

- [ ] **Step 1: Report ready-for-verification to the user**

Do NOT say "done" or "complete." State explicitly, in your own words:

> AGENTS.md is written and committed on `feature/agents-md`. Ready for your
> verification. Please check:
> - Open `AGENTS.md` and skim it top-to-bottom — does it read coherently?
> - Do the nine sections match what we agreed in the spec?
> - Are the repo paths and commands correct for your environment?
> - Does the Branch Workflow section capture the merge gate the way you want
>   (tests as the floor, you verify in the running app, merge only on your
>   word)?
>
> Tell me anything you want changed. I'll fix it on this same branch. I will
> not merge to `main` until you say "merge it."

- [ ] **Step 2: Wait for the user's response**

If the user requests changes → make them on the `feature/agents-md` branch,
re-commit, and re-hand off. Loop until the user says "merge it."

If the user says "merge it" → proceed to Task 14.

If the user reports a bug in the document (typo, wrong path, contradiction)
→ that's a fix on the same `feature/agents-md` branch (not a new `fix/`
branch — we haven't merged yet, so the feature branch is still the right
place).

---

### Task 14: Merge to main (only after the user says "merge it")
...
```bash
git checkout main && git pull
git merge --no-ff feature/agents-md -m "Merge branch 'feature/agents-md': AGENTS.md dev-workflow guide"
```
Expected: a merge commit is created on `main`.

- [ ] **Step 2: Push main if tracking remote**

Run:
```bash
git push origin main
```
If `origin` is configured this pushes the merge. If there's no remote
tracking, skip (the user may not want it pushed — ask if unsure).

- [ ] **Step 3: Delete the feature branch**

Run:
```bash
git branch -d feature/agents-md
git push origin --delete feature/agents-md 2>/dev/null || true
```
Expected: local branch deleted. Remote deletion skipped silently if it was
never pushed.

- [ ] **Step 4: Confirm final state**

Run: `git log --oneline -5`
Expected: the merge commit and the AGENTS.md commit are now on `main`.

Run: `git branch`
Expected: only `main` (and any other pre-existing branches); `feature/agents-md`
is gone.

Run: `test -f AGENTS.md && echo "AGENTS.md present on main"`
Expected: `AGENTS.md present on main`
